...
def generate_prompts(
    ...
    Returns formatted text suitable for the ComfyUI output widget, or an error
    string on failure.
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if parsed is None:
                return f"ERROR: Model returned empty or unparseable response.\nRaw: {getattr(response, 'text', 'N/A')}"

            return _format_output(parsed, len(images))
